...
if(!empty($template_data[0])>0) {
$data1 = $template_data[0];
$dpi = $data1['dpi'];
$canvaswidth = $data1['width'];//inch
$canvasheight = $data1['height'];//inch
...
<!--                    	<span class="loading-img"><img src="images/loading.gif"><br>LOADING...</span>-->
                ...
                <?php 
                if($data1['font1']!=''){
                    $fontfamily = str_replace('+', ' ', $data1['font1']);
                    ?>
                    <link href="https://fonts.googleapis.com/css?family=<?php echo $fontfamily; ?>" rel="stylesheet" type="text/css">
                    <?php
                ...
                <div class="canvas" style="position: relative;">
                    <div class="layer1" style= left:<?php echo $data1['bg_x']; ?>px;top:<?php echo $data1['bg_y']; ?>px;width:<?php echo $data1['bg_width']; ?>px;height:<?php echo $data1['bg_height']; ?>px;">
                        <?php if($data1['bg_img_h']){ 
                            if(file_exists(ASSETS_PATH.$data1['background_url'])){
                                $url = $this->tmodel->get_s3url($data1['background_url']);
                                // $url = ASSETS_URL.$data1['background_url'];
                            }
                            else{
                                $url = TEMPLATE_URL.$data1['background_url'];
                            }?>
                        <img src="<?php echo $url; ?>"  style="left:<?php echo $data1['bg_img_x']; ?>px;top:<?php echo $data1['bg_img_y']; ?>px;width:<?php echo $data1['bg_img_w']; ?>px;height:<?php echo $data1['bg_img_h']; ?>px;">
                        <?php }else{ ?>
                        <img src="<?php echo ASSETS_URL.$data1['background_url']; ?>"  style="position:relative;">
                        <?php } ?>
                    </div>
                    <div class="layer2" id="layer2">

                        <?php echo trim(preg_replace('/\s\s+/', ' ', file_get_contents($this->tmodel->get_s3url($data1['art_work_url'])))); 
                        //echo trim(preg_replace('/\s\s+/', ' ', file_get_contents(ASSETS_URL.$data1['art_work_url']))); 
                        ?>
                    </div>
                    <div class="logo" style="left:<?php echo $data1['logo_x']; ?>px;top:<?php echo $data1['logo_y']; ?>px;">
                        <?php if($data1['logo_img_h']){ 
                             if(file_exists(ASSETS_PATH.'logo/'.$data1['logo_url'])){
                                $url = ASSETS_URL.'logo/'.$data1['logo_url'];
                                $top=0;
                                $left = 0;
                            }
                            else{
                                $url = TEMPLATE_URL.$data1['logo_url'];
                                $top=$data1['logo_img_y'];
                                $left = $data1['logo_img_x'];
                            }
                           ?> 
                        <img src="<?php echo $url; ?>" >
                        <?php } else { ?>
                        <img src="<?php echo ASSETS_URL.'logo/'.$data1['logo_url']; ?>"  style="position:relative;">
                        <?php } ?>
                    </div>
                    <?php
                 ...
                 foreach($template_elements as $data)
                    { 
                        ...
                        $line_height = isset($font_settings->dstextheight)?'line-height:'.$font_settings->dstextheight.'px;':'';
                        $letter_spacing = isset($font_settings->dstextwidth)?'letter-spacing:'.$font_settings->dstextwidth.'px;':'';
                        ...
                        ?>
                        <div class="canvas_image canvas_elements" data-layer="<?php echo $data['element_name']; ?>" style='<?php echo "overflow:hidden;left:$left;top:$top;color: $color;font-size:$fontsize;text-align: $font_settings->dsalign;font-family: $fontfamily;$bold;$italic;$strikethrough;height:$height;width:$width; "; ?>'><span style="<?php echo "$underline;display:block;transform:$trans;-webkit-transform;$trans;$line_height $letter_spacing"; ?>"><?php echo $settings->dsdefaultcontent; ?></span></div>
...
//                                $content.='</ul>';
                            ?>
                        <div class="canvas_text canvas_elements" data-layer="<?php echo $data['element_name']; ?>"  style='<?php echo "overflow:hidden;left:$left;top:$top;color: $color;font-size: $fontsize;text-align: $font_settings->dsalign;font-family: $fontfamily;$bold;$italic;$strikethrough;height:$height;width:$width; "; ?>'><span style="<?php echo "$underline;display:block;transform:$trans;-webkit-transform;$trans; $line_height $letter_spacing"; ?>"><?php echo $content; ?></span></div>
                        <?php
...
<!--                        <input type="text" id="full-name" class="form-control" placeholder="123-456-7890" >-->
                        ...
                        if($data['element_type'] == 2){ 
                            if($data['element_name'] == "First Name") { 
                               $value =  isset($full_name['firstname']) && $full_name['firstname']!="" ? trim($full_name['firstname']) : ''; 
                                    }
                                    elseif($data['element_name'] == "Last Name") { 
                               $value =   isset($full_name['surname']) && $full_name['surname']!="" ? trim($full_name['surname']) : '';
                                    }
                                    elseif($data['element_name'] == "Phone - Office"){
                                       $value =  isset($user_info->office_phone) && $user_info->office_phone!="" ? $user_info->office_phone : ''; 
                                    }
                                      elseif($data['element_name'] == "Phone - Mobile"){
                                       $value = isset($user_info->mobile_phone) && $user_info->mobile_phone!="" ? $user_info->mobile_phone : '';
                                    }
                                     elseif($data['element_name'] == "Fax"){
                                       $value = isset($user_info->fax_number) && $user_info->fax_number!="" ? $user_info->fax_number : '';
                                    }
                                     elseif($data['element_name'] == "Email"){
                                     $value =  isset($user_info->user_email) && $user_info->user_email!="" ? $user_info->user_email : ''; 
                                    }
                                     elseif($data['element_name'] == "Website"){
                                        $value = isset($user_info->website) && $user_info->website!="" ? $user_info->website : ''; 
                                    }
                                    elseif($data['element_name'] == "Address"){
                                    $value = isset($user_info->address) && $user_info->address!="" ? $user_info->address : ''; 
                                    }
                                    elseif($data['element_name'] == "Zip"){
                                    $value =  isset($user_info->zipcode) && $user_info->zipcode!="" ? $user_info->zipcode : '';
                                    }
                                    elseif($data['element_name'] == "Bio"){
                                         $value = isset($user_info->biography) && $user_info->biography!="" ? $user_info->biography : ''; 
                                    }
                                    else{
                                        $value="";
                                    }
                                    if($data['input_type'] == 2){ ?>
                             <textarea placeholder="<?php echo $settings->dsdefaultcontent; ?>" maxlength="<?php echo $settings->dsmaxchars; ?>" class="form-control element_type" <?php echo $readonly; ?> data-element_type="<?php echo $data['element_type']; ?>"><?php echo $value;?></textarea>
                             <?php }else{ ?>
                            <input type="text" placeholder="<?php echo $settings->dsdefaultcontent; ?>" value="<?php echo $value;?>" maxlength="<?php echo $settings->dsmaxchars; ?>" class="form-control element_type" <?php echo $readonly; ?> data-element_type="<?php echo $data['element_type']; ?>">
                            <?php } ?>
                            ...
                            <?php } ?>
                            <?php }elseif($data['element_type'] == 1){ ?>
                ...
                <span class="input-group-btn">
                    <span class="btn btn-primary btn-file">
                        Upload <input type="file" class="element_image" multiple name="FileInput">
                            ...
                            <textarea placeholder="<?php echo $settings->dsdefaultcontent; ?>" maxlength="<?php echo isset($settings->dsmaxchars) && $settings->dsmaxchars!="" ? $settings->dsmaxchars : '200';?>" class="form-control element_type" <?php echo $readonly; ?> data-element_type="<?php echo $data['element_type']; ?>" data-bullet_type="<?php echo $settings->dsbullettype; ?>"></textarea>
                            <?php if(isset($settings->dsscalefont)){ ?>
                            <span class="dssettings-parent">
                                <input type="text" class="ds-fontpick" name="font_settings[]" value='<?php echo $data['font_settings']; ?>'>
                            </span>
                            ...
                            elseif($data['element_type'] == 5){
                                ?>
                                     <input type="hidden" placeholder="<?php echo $settings->dsdefaultcontent; ?>" value="<?php echo $settings->dsdefaultcontent; ?>" maxlength="" class="form-control element_type" <?php echo $readonly; ?> data-element_type="<?php echo $data['element_type']; ?>">
                                    <?php
                            }
                             elseif($data['element_type'] == 6){
                                ?>
                    ...
                    <div class="col-sm-12">
                      ...
                      <div class="col-sm-12">
                          ...
                          <select id="filetype" class="chosen-select form-control">
                              <?php if($data1['jpeg_export']){ ?>  <option value="jpeg">JPEG</option> <?php } ?>
                                <?php if($data1['png_export']){ ?><option value="png">PNG</option><?php } ?>
                                <?php if($data1['pdf_export']){ ?><option value="pdf">PDF</option><?php } ?>
                                    <?php if($data1['gif_export']){ ?><option value="gif">GIF</option><?php } ?>
                            </select>
<!--                      	<div class="checkbox-inline">
                      	<label><input type="checkbox" class="flat" checked="checked"> PDF</label>
                      </div> <div class="checkbox-inline">
                      	<label><input type="checkbox" class="flat" > JPG</label>
                      </div> <div class="checkbox-inline">
                      	<label><input type="checkbox" class="flat" checked="checked"> PNG</label>
                      </div> -->
                      </div>
                      <div class="col-sm-12">
...
<!--                <div style="border:1px solid #000;" id="edit_image"></div>-->
        <div style="border:1px solid #ccc; margin-top: 10px;" id="edit_image1"></div>
...
<canvas id="canvas" width="<?php echo $canvaswidth*$dpi; ?>" height="<?php echo $canvasheight*$dpi; ?>"></canvas>  
<script>
    ...
    //var svgText = document.getElementById("layer2").innerHTML;
...
//        svg = '<?php // echo trim(preg_replace('/\s\s+/', ' ', file_get_contents($data1['art_work_url']))); ?>';
//        canvg(document.getElementById('layer2'), svg);
$('.color1').css('fill', '<?php echo $data1['color1'];?>');
$('.color2').css('fill', '<?php echo $data1['color2'];?>');
$('.color3').css('fill', '<?php echo $data1['color3'];?>');
$('.color4').css('fill', '<?php echo $data1['color4'];?>');
...
$('.layer2 svg').css('transform','none');
            svgText = $('.layer2').html();
         
			swidth = width*dpi;
			sheight = height*dpi;
			
			$.post(SITE_URL + 'template/svgtopng', {'svg_content': svgText,'swidth': swidth, 'sheight': sheight}, function (res) {
           $('.layer2').html('<img src="'+res+'" style="width:'+width+';height:'+height+'">');
        });
      ...
      //  canvas.css('zoom', zoom+'%' );
        ...
        $('.element_type').on('keyup change', function(){
            ...
            if(element_type==2){
                $('[data-layer="'+source+'"] span').html($(this).val().replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)/g, '$1'+ '<br>' +'$2'));
        ...
        $('#download').on('click', function(){
            ...
            clone.children('.canvas_text').each(function(i,v){
                ...
                if($.trim($('[data-source="'+source+'"]').find('input,textarea').val())=='' && $('[data-source="'+source+'"]').length!=0){
                    $(this).remove();
                }
            });
            clone.appendTo( "body");
            ...
            html2canvas(clone, {
                onrendered: function(canvas) {
                    ...
                    if(filetype=='pdf'){
                        ...
                        var imgData = theCanvas.toDataURL("image/jpeg", 1.0);
                        var pdf = new jsPDF('', 'in', [width, height]);
