      ...
      <div class="row">
        <div class="col-sm-12">
          <div class="expire_subs">
            ...
            <p>Kindly Pay to reactivate your subscription so that you can resume using Brandize features uninterrupted.</p>
          </div>
        </div>
      </div>
      <?php
 $attributes = array('id' => 'activate_frm','class' => 'regration-form form-horizontal');
 echo form_open(COMPANY_URL."update_subscription",$attributes);?>
					...
					if(returnjson.type=="success"){
						...
						if(coupon_type=="amount"){
							...
							$('#coupon').html('$' + new_amount.toFixed(2) + ' ' + plan_type <?php if(isset($tax) && $tax>0){?> +'<small>+ <?php echo $tax;?>% Tax of $'+new_amount.toFixed(2)+'</small>'<?php } ?>).fadeIn();
								 ...
								 $("#coupon").html('$' + new_amount.toFixed(2) + ' ' + plan_type <?php if(isset($tax) && $tax>0){?> +'<small>+ <?php echo $tax;?>% Tax of $'+new_amount.toFixed(2)+'</small>'<?php } ?>)
							 }
						}
