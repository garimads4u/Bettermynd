<div class="col-sm-12">
  <h2 class="page-heading">Make Payment</h2>
  <?php if(isset($puser_id) && $puser_id>0){
?>
  <div class="x_panel">
    <div class="x_content">
      <div class="pull-right"> <a href="<?php echo COMPANY_URL;?>user_transaction/<?php echo base64_encode($puser_id);?>" class="btn btn-primary">Back To Transaction</a> </div>
    </div>
  </div>
  <?php
			}
?>
  <div class="x_panel">
    <div class="x_content"> <?php echo form_input($user_type);?>
      <?php
 $attributes = array('id' => 'cpayment_frm','class' => 'regration-form form-horizontal');
 echo form_open(COMPANY_URL."pay_user_subscription",$attributes);?>
      <?php echo form_input($user_type); 
	  if(isset($puser_id) && $puser_id>0){
		  ?>
      <input type="hidden" name="puser_id" id="puser_id" value="<?php echo $puser_id;?>" />
      <?php
	  }
	  ?>
      <div class="row">
                  ...
                  <div role="tabpanel" class="tab-pane fade active in" id="tab_content1" aria-labelledby="profile-tab">
                    <div>
                      <div class="card_total row">
                        <div class="col-sm-6"> <strong>TOTAL: </strong><span id='coupon' ><?php echo CURRENCY_SYMBOL;?><?php echo number_format($packages[0]['package_amount'],2);?> <?php echo ucwords(strtolower($packages[0]['package_mode']));?> </span>
                          <input type="hidden" name="final_amount" id="final_amount" value="<?php echo $packages[0]['package_amount'];?>" />
                        </div>
                        <div class="col-sm-6"> <a title="Remove Coupon Code" href="javascript:void(0);" class="mandatory hide" id="btnclear" style="float:left !important;margin-left:-13px !important;margin-top:10px !important;"><i class="fa fa-times"></i></a> <a href="javascript:void(0)" class="btn-apply">Apply Discount Code</a>
	...
	$("#final_amount").val($(this).data('price'));
	  ...
	  $("#coupon_btn").click(function(e) {
			...
			var returnjson=JSON.parse(data);
					if(returnjson.type=="success"){
						...
						if(coupon_type=="amount"){
							...
							$('#coupon').text('$' + new_amount + ' '+plan_type).fadeIn();
								 ...
								 $("#coupon").text('$' + net_amount + ' ' +plan_type + " (  "+returnjson.percentage+"% Off)")
							 }
						}
					...
					$("#coupon_code").css("background-color","#f2f3f3 !important");
