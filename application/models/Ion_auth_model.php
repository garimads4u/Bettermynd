...
class Ion_auth_model extends CI_Model {
    ...
    public function hash_password_db($id, $password, $use_sha1_override = FALSE) {
        ...
        $query = $this->db->select('user_pswd, salt')
                ->where('user_id', $id)
                ->limit(1)
        ...
        // bcrypt
        if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt') {
            $this->bcrypt->verify($password, $hash_password_db->user_pswd);

            if ($this->bcrypt->verify($password, $hash_password_db->user_pswd)) {
                return TRUE;
            }
        ...
        if ($this->store_salt) {
            ...
            $salt = substr($hash_password_db->user_pswd, 0, $this->salt_length);

            $db_password = $salt . substr(sha1($salt . $password), 0, -$this->salt_length);
        }

        if ($db_password == $hash_password_db->user_pswd) {
            return TRUE;
        } else {
            return FALSE;
        }
    ...
    public function reset_password($identity, $new) {
        ...
        $query = $this->db->select('user_id, user_pswd, salt')
    ...
    public function forgotten_password($identity) {
        ...
        $pass_data = array('user_email' => $identity);


        $this->db->update($this->tables['users'], $update, $pass_data);
    ...
    public function forgotten_password_complete($code, $salt = FALSE) {
        ...
        if ($profile) {
            ...
            $data = array(
                'user_pswd' => $this->hash_password($password, $salt),
                'forgotten_password_code' => NULL,
                'user_status' => 1,
            );
    ...
    public function register($identity, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        if (isset($default_group->user_id) && empty($groups)) {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column . ', user_email, user_id,user_type, user_pswd, user_status, user_last_loggedon, is_disabled')
                ->where($this->identity_column, $identity)
                ->limit(1)
                ->order_by('user_id', 'desc')
                ->get($this->tables['users']);
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                /* if ($user->user_status == 0) {
                  ...
                  //                        $this->trigger_events('post_login_unsuccessful');
                  //
                  //                        $this->set_error('login_unsuccessful_not_approved');
                  //
                  //                        return FALSE;
                  //} else {
                  // $this->trigger_events('post_login_unsuccessful');
                  ...
                  $this->set_error('login_unsuccessful_disabled');
                  }
                  return FALSE;
                  } */

                if ($user->user_pswd == '0' && $user->user_status == '0') {
                    $this->trigger_events('post_login_unsuccessful');
                    $this->set_error('login_unsuccessful_not_active');
                    return FALSE;
                }
    ...
    public function users($groups = NULL) {
        ...
        if (isset($groups)) {
            ...
            if (isset($groups) && !empty($groups)) {
                $this->db->distinct();
                $this->db->join(
                        $this->tables['users_groups'], $this->tables['users_groups'] . '.' . $this->join['users'] . '=' . $this->tables['users'] . '.user_id', 'inner'
                );
            }
    ...
    public function user($id = NULL) {
        ...
        $this->where($this->tables['users'] . '.user_id
		', $id);
    ...
    public function update_last_login($id) {
        ...
        $current_time = $this->basic_model->set_utc_datetime(date('Y-m-d H:i:s'), 1);

        $this->db->update($this->tables['users'], array('user_last_loggedon' => $current_time), array('user_id' => $id));
    ...
    public function set_session($user) {
        ...
        $session_data = array(
            ...
            'old_last_login' => $user->user_last_loggedon
        );
    ...
    public function login_remembered_user() {
        ...
        // get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', user_id, user_email, user_last_loggedon')
