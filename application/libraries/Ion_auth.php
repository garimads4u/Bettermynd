...
class Ion_auth {
    ...
    public function __construct() {
        ...
        $this->load->helper(array('cookie', 'language', 'url', 'email'));
    ...
    public function forgotten_password($identity) {    //changed $email to $identity
        if ($this->ion_auth_model->forgotten_password($identity)) {   //changed
            ...
            $identifier = 'user_email'; // use model identity column, so it can be overridden in a controller
            $user = $this->where($identifier, $identity)->where('user_status', 1)->users()->row();  // changed to get_user_by_identity from email
            ...
            if ($user) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
                    ...
                    $message_content = $this->basic_model->get_mail_template('forgot_password');
    ...
    public function register($identity, $password, $email, $additional_data = array(), $group_ids = array()) { //need to test email activation
        ...
        if (!$email_activation) {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                if ($additional_data['user_type'] == "2") {
                    $message_content = $this->basic_model->get_mail_template('activation_mail_college');
                    $mail_content = html_entity_decode($message_content->mail_template_content);
                    $message = str_replace("{{identity}}", $additional_data['first_name'] . " " . $additional_data['last_name'], $mail_content);
                    $message = str_replace("{{siteurl}}", SITE_URL, $message);
                    $message = str_replace("{{id}}", $data['id'], $message);
                    $message = str_replace("{{activationcode}}", $data['activation'], $message);
                    $message = MAIL_HEADER . $message . MAIL_FOOTER;
...
//                    $this->email->message($message);
                ...
                if ($additional_data['user_type'] == "3") {
                    $message_content = $this->basic_model->get_mail_template('activation_mail_provider');
                    $mail_content = html_entity_decode($message_content->mail_template_content);
                    $message = str_replace("{{identity}}", $additional_data['first_name'] . " " . $additional_data['last_name'], $mail_content);
                    $message = str_replace("{{siteurl}}", SITE_URL, $message);
                    $message = str_replace("{{id}}", $data['id'], $message);
...
//                    $this->email->message($message);
                ...
                if ($additional_data['user_type'] == "4") {
                    $message_content = $this->basic_model->get_mail_template('activation_mail_patient');
                    $mail_content = html_entity_decode($message_content->mail_template_content);
                    $message = str_replace("{{identity}}", $additional_data['first_name'] . " " . $additional_data['last_name'], $mail_content);
                    $message = str_replace("{{siteurl}}", SITE_URL, $message);
                    $message = str_replace("{{id}}", $data['id'], $message);
...
//                    $this->email->message($message);
                ...
                if ($additional_data['user_type'] == "5") {
                    $message_content = $this->basic_model->get_mail_template('activation_mail_third_party');
                    $mail_content = html_entity_decode($message_content->mail_template_content);
                    $message = str_replace("{{identity}}", $additional_data['first_name'] . " " . $additional_data['last_name'], $mail_content);
                    $message = str_replace("{{siteurl}}", SITE_URL, $message);
                    $message = str_replace("{{id}}", $data['id'], $message);
...
//                    $this->email->message($message);
                ...
                if (isset($message_content) && !empty($message_content)) {
                    ...
                    if ($this->custom_email->sendinblue_email($this->config->item('admin_email', 'ion_auth'), $email, $message_content->mail_subject, $message) == TRUE) {
                        $admin = $this->basic_model->getAdminDetails();
                        /*                         * ******* Send email to Administrator ****************** */
                        $user_type = '';
                        if ($additional_data['user_type'] == "3") {
                            $user_type = 'Counselor';
                        } else if ($additional_data['user_type'] == "5") {
                            $user_type = 'Third Party Counselor';
                        } else if ($additional_data['user_type'] == "4") {
                            $user_type = 'Student';
                        } else if ($additional_data['user_type'] == "2") {
                            $user_type = 'College';
                        }
                        if ($user_type != '') {
                            $message_content = $this->basic_model->get_mail_template('admin_mail_new_registration');
                            $mail_content = str_replace('  ', ' ', html_entity_decode($message_content->mail_template_content));
                            $message = str_replace("{{identity}}", ucfirst($admin['first_name'] . " " . $admin['last_name']), $mail_content);
                            $message = str_replace("{{NAME}}", ucfirst($additional_data['first_name'] . " " . $additional_data['last_name']), $message);
                            $message = str_replace("{{EMAIL}}", $email, $message);
                            $message = str_replace("{{ROLE}}", $user_type, $message);
                            $message = MAIL_HEADER . $message . MAIL_FOOTER;
...
//                            $this->email->message($message);
//                            $this->email->send();
                            $this->custom_email->sendinblue_email($this->config->item('admin_email', 'ion_auth'), $admin['user_email'], $message_content->mail_subject, $message);
                        }
                        $this->set_message('activation_email_successful');
                        return $id;
                    }
    ...
    public function register_user($identity, $password, $email, $additional_data = array(), $group_ids = array()) { //need to test email activation
        $this->ion_auth_model->trigger_events('pre_account_creation');
        ...
        if (!$email_activation) {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                $this->ion_auth_model->trigger_events(array('post_account_creation', 'post_account_creation_successful', 'activation_email_send'));
                $this->set_message('activation_email_send');
                return $data;
            } else {
