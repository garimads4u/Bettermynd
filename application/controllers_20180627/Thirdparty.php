...
class Thirdparty extends CI_Controller {
    ...
    function dashboard() {
        if (!$this->ion_auth->logged_in()) {
            ...
            if (isset($user_complete_status) && $user_complete_status != 1) {
                redirect(THIRD_PARTY_URL . "manage_profile");
            } else {
                $this->data = array();
                $this->sidebardata = array();
                $limit = 10;
                $this->data['script_to_include'] = "thirdparty.js";
                $this->data['total_transaction_amount'] = $this->tp->get_total_transaction($current_user->user_id);
                $this->data['total_upcomming_appointment'] = $this->provider_model->get_total_upcomming_appointment($current_user->user_id);
                $this->data['total_appointment'] = $this->tp->get_total_appointment($current_user->user_id);
                $this->data['total_patient'] = $this->tp->get_total_patient($current_user->user_id);
                $this->data['our_patient'] = $this->tp->get_our_patient($current_user->user_id);
                $this->data['upcoming_appointment'] = $this->tp->get_upcoming_appointment($current_user->user_id, $limit);
                $this->data['past_appointment'] = $this->tp->get_past_appointment($current_user->user_id, $limit);
                $this->headerdata['page_title'] = "Dashboard";
                $this->headerdata['breadcrumbs'] = array(THIRD_PARTY_URL, array('dashboard', 'Dashboard'));
                ...
                $this->elements['middle'] = 'thirdparty/dashboard';
    ...
    function upcoming_appoinment() {
        if (!$this->ion_auth->logged_in()) {
            ...
            if (isset($user_complete_status) && $user_complete_status != 1) {
                ...
                $this->sidebardata = array();
                $this->data['script_to_include'] = "thirdparty.js";
                $this->data['upcoming_appointment'] = $this->tp->get_upcoming_appointment($current_user->user_id);
                $this->headerdata['page_title'] = "Upcoming Appoinment";
                $this->headerdata['breadcrumbs'] = array(THIRD_PARTY_URL, array('dashboard', 'Dashboard'), array('upcoming_appoinment', 'Upcoming Appoinment'));
                $this->sidebardata["profile_photo"] = $current_user->profile_image;
                $this->sidebardata["username"] = ucwords($current_user->first_name . ' ' . $current_user->last_name);
                $this->sidebardata['is_active'] = 'dashboard';
                ...
                $this->templatelayout->get_admin_dashboard_footer();
                $this->templatelayout->get_admin_dashboard_sidebar('thirdpartysidebar', $this->sidebardata);
                $this->elements['middle'] = 'thirdparty/upcoming_appoinment';
                $this->elements_data['middle'] = $this->data;
                $this->layout->setLayout('backend_layout/dashboardlayout');
                $this->layout->multiple_view($this->elements, $this->elements_data);
            }
    ...
    public function manage_profile() {
        ...
        $profile_data = $this->tp->getProfileData($current_user->user_id);
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            $this->data['user_email'] = array(
                ...
                'value' => isset($postdata) && !empty($postdata) && isset($postdata['user_email']) ? $postdata['user_email'] : $profile_data->user_email
            );
            ...
            $this->data['first_name'] = array(
                ...
                'value' => isset($postdata) && !empty($postdata) && isset($postdata['first_name']) ? $postdata['first_name'] : $profile_data->first_name
            );
            $this->data['last_name'] = array(
                ...
                'value' => isset($postdata) && !empty($postdata) && isset($postdata['last_name']) ? $postdata['last_name'] : $profile_data->last_name
            );
            $this->data['address'] = array(
                ...
                'value' => isset($postdata) && !empty($postdata) && isset($postdata['address']) ? $postdata['address'] : $profile_data->address
            );
            $this->data['city'] = array(
                ...
                'value' => isset($postdata) && !empty($postdata) && isset($postdata['city']) ? $postdata['city'] : $profile_data->city
            );
            ...
            $this->data['zipcode'] = array(
                ...
                'value' => isset($postdata) && !empty($postdata) && isset($postdata['zipcode']) ? $postdata['zipcode'] : $profile_data->zipcode
            );
            $this->data['mobile_no'] = array(
                ...
                'value' => isset($postdata) && !empty($postdata) && isset($postdata['mobile_no']) ? $postdata['mobile_no'] : $profile_data->mobile_no
            );
            $this->data['biography'] = array(
                ...
                'value' => isset($postdata) && !empty($postdata) && isset($postdata['biography']) ? $postdata['biography'] : $profile_data->biography
            );
            ...
            $this->data['session_cost'] = array(
                ...
                'placeholder' => "Average rate per 45 minute session ($)",
                'value' => isset($postdata) && !empty($postdata) && isset($postdata['session_cost']) ? $postdata['session_cost'] : $profile_data->session_cost
            );
            ...
            if (isset($profile_data->state) && !empty($profile_data->state)) {
                $state_select = $profile_data->state;
            } else {
                $state_select = '';
            }
            ...
            if (isset($profile_data->specialities) && !empty($profile_data->specialities)) {
                $profileSpeciality = explode(',', $profile_data->specialities);
            }

            if (isset($profile_data->insurance_carriers) && !empty($profile_data->insurance_carriers)) {
                $profileInsurance = explode(',', $profile_data->insurance_carriers);
            }

            $this->data['timezone_list'] = $this->basic_model->get_timezonelist();
            if (isset($profile_data->timezone_id) && !empty($profile_data->timezone_id)) {
                $timezone_id = $profile_data->timezone_id;
            } else {
            ...
            $this->data['profile_image'] = $profile_data->profile_image;
            $this->data['photo_id'] = $profile_data->photo_id;
            ...
            $this->sidebardata["profile_photo"] = $current_user->profile_image;
            $this->sidebardata["role_name"] = $this->basic_model->get_user_role($current_user->user_type);
            $this->headerdata['page_title'] = "Update Profile";

            $this->data['script_to_include'] = "thirdparty.js";
            $this->data['current_user'] = $current_user;

            $this->headerdata['breadcrumbs'] = array(THIRD_PARTY_URL, array('dashboard', 'Dashboard'), array('manage_profile', 'Manage Profile'));
            $this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
            $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
            $this->templatelayout->get_admin_dashboard_header($this->headerdata);
            $this->templatelayout->get_admin_dashboard_footer();
            $this->templatelayout->get_admin_dashboard_sidebar('thirdpartysidebar', $this->sidebardata);
            $this->elements['middle'] = 'thirdparty/thirdparty_manage_profile';
            $this->elements_data['middle'] = $this->data;
            $this->layout->setLayout('backend_layout/dashboardlayout');
            $this->layout->multiple_view($this->elements, $this->elements_data);
        }
    ...
    public function manage_thirdparty_profile() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->tp->update_profile($post_arr)) {
                $this->session->set_flashdata('message', $this->lang->line('profile_update_provider'));
                redirect(THIRD_PARTY_URL . "manage_profile");
            } else {
                $_SESSION['postdata'] = $_POST;
                $this->session->set_flashdata('error', "Accept extension only JPG/PNG/JPEG/DOC/PDF type format.");
                redirect(THIRD_PARTY_URL . "manage_profile");
            }
        } else {
            $_SESSION['postdata'] = $_POST;
    ...
    public function view_profile() {
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            $this->headerdata = array();
            if (isset($profile_data->specialities) && !empty($profile_data->specialities)) {
                $speciality = $this->basic_model->getSpecialityName($profile_data->specialities);
            } else {
                $speciality = "N/A";
            }
            ...
            $this->data['timezone'] = $this->basic_model->getTimeZoneName($profile_data->timezone_id);
            $certificate_uploads = $this->tp->getthirdparty_counseling_certifications($current_user->user_id);
            $insurance_certificate_upload = "";
            if (isset($certificate_uploads) && !empty($certificate_uploads)) {
                foreach ($certificate_uploads as $upload) {
            ...
            $this->sidebardata = array();
            $this->sidebardata["username"] = ucwords($current_user->first_name . ' ' . $current_user->last_name);
            $this->sidebardata['is_active'] = 'view_profile';

            $this->sidebardata["profile_photo"] = $current_user->profile_image;
            $this->headerdata['page_title'] = "View Profile";
            $this->headerdata['breadcrumbs'] = array(THIRD_PARTY_URL, array('dashboard', 'Dashboard'), array('view_profile', 'View Profile'));
            $this->templatelayout->get_admin_dashboard_header($this->headerdata);
            $this->templatelayout->get_admin_dashboard_footer();
            $this->templatelayout->get_admin_dashboard_sidebar('thirdpartysidebar', $this->sidebardata);
            $this->elements['middle'] = 'thirdparty/view_profile';
            $this->elements_data['middle'] = $this->data;
            $this->layout->setLayout('backend_layout/dashboardlayout');
            $this->layout->multiple_view($this->elements, $this->elements_data);
        }
    ...
    public function change_password() {
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            $this->headerdata['breadcrumbs'] = array(THIRD_PARTY_URL, array('dashboard', 'Dashboard'), array('changepassword', 'Change Password'));
            $this->templatelayout->get_admin_dashboard_header($this->headerdata);
            $this->templatelayout->get_admin_dashboard_footer();
            $this->templatelayout->get_admin_dashboard_sidebar('thirdpartysidebar', $this->sidebardata);
            $this->elements['middle'] = 'thirdparty/change_password';
            $this->elements_data['middle'] = $this->data;
            $this->layout->setLayout('backend_layout/dashboardlayout');
            $this->layout->multiple_view($this->elements, $this->elements_data);
        }
    ...
    public function user_change_password() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($change) {
                ...
                if (isset($message_content) && !empty($message_content)) {
                    ...
                    if ($this->email->send() == TRUE) {
                        ...
                        redirect(THIRD_PARTY_URL . "change_password", 'refresh');
                    } else {
                        $this->session->set_flashdata('message', $this->ion_auth->messages());
                        redirect(THIRD_PARTY_URL . "change_password", 'refresh');
                    }
            ...
            $this->session->set_flashdata('error', (validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message'))));
            redirect(THIRD_PARTY_URL . "change_password");
        }
    }
    ...
    public function availability() {
        if (!$this->ion_auth->logged_in()) {
            ...
            $this->headerdata['breadcrumbs'] = array(THIRD_PARTY_URL, array('dashboard', 'Dashboard'), array('availabality', 'Availability'));
            $this->templatelayout->get_admin_dashboard_header($this->headerdata);
            $this->templatelayout->get_admin_dashboard_footer();
            $this->templatelayout->get_admin_dashboard_sidebar('thirdpartysidebar', $this->sidebardata);
            $this->elements['middle'] = 'thirdparty/availabality';
            $this->elements_data['middle'] = $this->data;
            $this->layout->setLayout('backend_layout/dashboardlayout');
            $this->layout->multiple_view($this->elements, $this->elements_data);
        }
    ...
    public function saveavailabality() {
        ...
        if ($count['avl_count'] > 0) {
            ...
            redirect('thirdparty/availability');
        }
...
// Loop between timestamps, 24 hours at a time
    ...
    public function update_schedule() {
        ...
        $this->load->view('thirdparty/update_schedule', $this->data);
    }
            ...
            if (isset($ttype) && !empty($ttype)) {
                if ($ttype == "appointment" && isset($patient_id) && !empty($patient_id)) {
                    ...
                    if ($update) {
                        ...
                        redirect(THIRD_PARTY_URL . 'availability');
                    }
                } else {
    ...
    public function past_appoinment() {
        if (!$this->ion_auth->logged_in()) {
            ...
            if (isset($user_complete_status) && $user_complete_status != 1) {
                ...
                $this->sidebardata = array();
                $this->data['script_to_include'] = "thirdparty.js";
                $this->data['past_appointment'] = $this->tp->get_past_appointment($current_user->user_id);
                $this->headerdata['page_title'] = "Past Appoinments";
                $this->headerdata['breadcrumbs'] = array(THIRD_PARTY_URL, array('dashboard', 'Dashboard'), array('past_appoinment', 'Past Appoinment'));
                $this->sidebardata["profile_photo"] = $current_user->profile_image;
                $this->sidebardata["username"] = ucwords($current_user->first_name . ' ' . $current_user->last_name);
                $this->sidebardata['is_active'] = 'dashboard';
                ...
                $this->templatelayout->get_admin_dashboard_footer();
                $this->templatelayout->get_admin_dashboard_sidebar('thirdpartysidebar', $this->sidebardata);
                $this->elements['middle'] = 'thirdparty/past_appoinment';
                $this->elements_data['middle'] = $this->data;
                $this->layout->setLayout('backend_layout/dashboardlayout');
                $this->layout->multiple_view($this->elements, $this->elements_data);
            }
